...
: Daily to pentad1/2 (using Numpy)
: Daily to monthly1 (using Numpy)
: Daily to monthly2 (using Pandas)
...
def daily_to_pentad2(ts1d,xt,tgt_dates,tdelta=5):
    ...
    while(date1+timedelta(days=365)<=tgt_dates[1]):
        ...
        date1= date(date1.year+1,date1.month,date1.day)
...
def daily_to_monthly1(ts1d,tgt_dates):
    ...
    while(True):
        ...
        if ndy>20:  # minimum days to represent a month
            ...
            xt_mm.append(date(yr1,mm1,int(ndy/2+0.5)))
            date1= date2
        else:
            break
...
def plot_main(pdata):
    ...
    for i,(tit,(xt,ydata)) in enumerate(zip(titles,ts_data)):
        ...
        ## Line plots
        ax1.plot(dx,dy,c='C1',lw=1,alpha=0.9,label='Daily')
        ax1.plot(xt,ydata,c='C0',lw=3,alpha=0.6,label=tit.split()[0])

        subtit= '({}) {}'.format(abc[i],tit)
        plot_common(ax1,subtit)
        ax1.legend(loc='upper left',bbox_to_anchor=(1.02,1.),borderaxespad=0.,edgecolor='k',
                    fontsize=10,prop=dict(stretch='semi-condensed'))
