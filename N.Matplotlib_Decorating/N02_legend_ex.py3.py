...
for i in range(nrow*ncol):
    ...
    if i<10:
        ax1.legend(loc=i+1,bbox_to_anchor=(0.5,0.5),fontsize=9,framealpha=0.7)
        subtit='({}) Legend Loc={}'.format(abc[i],i+1)
    elif i==10:
        subtit='({}) Legend Loc=9, ncol=2'.format(abc[i])
        ax1.legend(loc=9,bbox_to_anchor=(0.5,1.),fontsize=9,framealpha=0.7,ncol=2)        
    else:
        subtit='({}) Legend Loc=3'.format(abc[i])
        ax1.legend(loc=2,bbox_to_anchor=(1.05,1.),fontsize=9,borderaxespad=0.) #,framealpha=0.7)
